...
The study found a technically credible conditional stack built around ASE, CP2K, and AiiDA. xTB and Psi4 have
limited validation roles. Sella and OVITO are optional for later pathway analysis and visualization. The stack is
conditional because the public paper does not provide the model coordinates, QM/MM partition, boundary treatment,
basis and ECP choices, coupling scheme, or complete driven-coordinate protocol. No open-source implementation has
yet been shown equivalent to the unpublished setup.
...
Faithful reproduction is **blocked** until the missing trajectory inputs and method-parity evidence are available.
A declared-deviation study could test whether the reported qualitative event order appears under a fully specified
open-source setup, but that would be a new computational prediction rather than a reproduction of the Cowie
calculation. The scientific gate remains **pending**, and this package authorizes no calculation.
...
| GFN0/DFT two-level calculation | GFN0-xTB, omega-B97X-D3, coupled energies and forces | CP2K, with xTB and Psi4 used for bounded validation | Partition, embedding, link treatment, basis, ECP, and coupling scheme are unavailable or unvalidated |
